// ----------------------------------------------------------------------------
// vtkLegiPolyDataMapper.cpp : extension to vtkOpenGLPolyDataMapper for 
...
#ifndef VTK_IMPLEMENT_MESA_CXX
vtkStandardNewMacro(vtkLegiPolyDataMapper);
#endif
...
vtkLegiPolyDataMapper::vtkLegiPolyDataMapper()
{
  this->ListId = 0;
  this->TotalCells = 0;
...
void vtkLegiPolyDataMapper::ReleaseGraphicsResources(vtkWindow *win)
{
...
//
void vtkLegiPolyDataMapper::RenderPiece(vtkRenderer *ren, vtkActor *act)
{
...
void vtkLegiPolyDataMapper::DrawPoints(int idx,
                                         vtkPoints *p, 
                                         vtkDataArray *n,
...
void vtkLegiPolyDataMapper::DrawLines(int idx,
                                        vtkPoints *p, 
                                        vtkDataArray *n,
                                        vtkUnsignedCharArray *c,
                                        vtkDataArray *t,
...
void vtkLegiPolyDataMapper::DrawPolygons(int idx,
                                         vtkPoints *p, 
                                         vtkDataArray *n,
                                         vtkUnsignedCharArray *c,
                                         vtkDataArray *t,
...
void vtkLegiPolyDataMapper::DrawTStrips(int idx,
                                        vtkPoints *p, 
                                        vtkDataArray *n,
                                        vtkUnsignedCharArray *c,
                                        vtkDataArray *t,
...
void vtkLegiPolyDataMapperDrawTStripLines(int idx,
                                            vtkPoints *p, 
                                            vtkDataArray *n,
                                            vtkUnsignedCharArray *c,
                                            vtkDataArray *t,
...
int vtkLegiPolyDataMapper::Draw(vtkRenderer *aren, vtkActor *act)
{
  ...
  //cout << "------------- start drawing polygonal data here -------------- \n";
  // get the property 
  ...
  if (rep == VTK_POINTS && !prop->GetBackfaceCulling() && !prop->GetFrontfaceCulling() )
    ...
    vtkLegiPolyDataMapperDrawTStripLines(idx,p,n,c,t,oldCellNum, noAbort,
  ...
  if (prop->GetFrontfaceCulling() /*|| prop->GetBackfaceCulling()*/) {
	  /*
	  static int i = 0;
	  if ( i<=1 ) {
		  cout << "--- restored here in vtkLegiPolyDataMapper ---\n";
		  glPopAttrib();
	  }
	  i++;
	  */
  ...
  //cout << "------------- end drawing polygonal data here -------------- \n";
  return noAbort;
}

void vtkLegiPolyDataMapper::PrintSelf(ostream& os, vtkIndent indent)
{
