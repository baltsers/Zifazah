// ----------------------------------------------------------------------------
// vtkThresholdedPolyDataFilteredPolyDataFilter.h : an extension to vtkThresholdedPolyDataFilter
//	for customizing output as polyData rather than vtkUnstructuredGrid
//
// Creation : Feb. 24th 2012
// revision : 
//	-- add random candidate selection filter ; March 19, 2012
//
// Author: 2012-2013 Haipeng Cai
//
// ----------------------------------------------------------------------------
//
#ifndef __vtkThresholdedPolyDataFilteredPolyDataFilter_h
#define __vtkThresholdedPolyDataFilteredPolyDataFilter_h
...
class VTK_GRAPHICS_EXPORT vtkThresholdedPolyDataFilter : public vtkPolyDataAlgorithm
{
public:
  static vtkThresholdedPolyDataFilter *New();
  vtkTypeMacro(vtkThresholdedPolyDataFilter, vtkPolyDataAlgorithm);
  ...
  void ThresholdByMatch(const std::set<double>& candidates);
  void ThresholdByMatch();

  // Description:
/*=========================================================================

  Program:   Visualization Toolkit
  Module:    vtkThresholdedPolyDataFilter.h

  Author: (c) Ken Martin, Will Schroeder, Bill Lorensen
  All rights reserved.
  See Author:.txt or http://www.kitware.com/Author:.htm for details.
...
=========================================================================*/
// .NAME vtkThresholdedPolyDataFilter - extracts cells where scalar value in cell satisfies threshold criterion
// .SECTION Description
// vtkThresholdedPolyDataFilter is a filter that extracts cells from any dataset type that
...
// .SECTION See Also
// vtkThresholdedPolyDataFilterPoints vtkThresholdedPolyDataFilterTextureCoords
  ...
  void addCandidate(double s) { Candidates.insert( s ); this->Modified(); }
  void removeCandidate(double s) { Candidates.erase(s); this->Modified(); }
  void clearCandidates() { Candidates.clear(); this->Modified(); }

protected:
  vtkThresholdedPolyDataFilter();
  ~vtkThresholdedPolyDataFilter();
  ...
  //BTX
  int (vtkThresholdedPolyDataFilter::*ThresholdFunction)(double s);
  //ETX
  ...
  int IsCandidate(double s) { return Candidates.end() != Candidates.find( s ); }
...
private:
  vtkThresholdedPolyDataFilter(const vtkThresholdedPolyDataFilter&);  // Not implemented.
  void operator=(const vtkThresholdedPolyDataFilter&);  // Not implemented.
};
