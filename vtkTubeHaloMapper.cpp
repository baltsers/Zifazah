...
#ifndef VTK_IMPLEMENT_MESA_CXX
vtkStandardNewMacro(vtkTubeHaloMapper);
#endif
...
vtkTubeHaloMapper::vtkTubeHaloMapper()
{
	cout << "mapper created.\n";
}
...
//
void vtkTubeHaloMapper::RenderPiece(vtkRenderer *ren, vtkActor *act)
{
...
void vtkTubeHaloMapper::DrawPoints(int idx,
                                         vtkPoints *p, 
                                         vtkDataArray *n,
...
void vtkTubeHaloMapper::DrawLines(int idx,
                                        vtkPoints *p, 
                                        vtkDataArray *n,
                                        vtkUnsignedCharArray *c,
                                        vtkDataArray *t,
...
void vtkTubeHaloMapper::DrawPolygons(int idx,
                                         vtkPoints *p, 
                                         vtkDataArray *n,
                                         vtkUnsignedCharArray *c,
                                         vtkDataArray *t,
...
void vtkTubeHaloMapper::DrawTStrips(int idx,
                                        vtkPoints *p, 
                                        vtkDataArray *n,
                                        vtkUnsignedCharArray *c,
                                        vtkDataArray *t,
                                        ...
                                        vtkRenderer *ren)
{
  ...
  if (c)
    {
    colors = c->GetPointer(0);
    cout << "#tuples=" << c->GetNumberOfTuples() << "\n";
    cout << "#components=" << c->GetNumberOfComponents() << "\n";
    }
  if (t)
    {
    voidTCoords = t->GetVoidPointer(0);
  ...
  vtkIdType *ptIds = ca->GetPointer();
  vtkIdType *endPtIds = ptIds + ca->GetNumberOfConnectivityEntries();
  cout << "connectivity numer: " << ca->GetNumberOfConnectivityEntries() << "\n";
  ...
  switch (idx)
    ...
    case VTK_PDM_POINT_TYPE_FLOAT|VTK_PDM_NORMAL_TYPE_FLOAT|VTK_PDM_NORMALS:
      ...
      vtkDrawPolysMacro(float, float, float, rep,
                        ...
                        glNormal3fv(normals + 3**ptIds);
						*/
                        _point_t<GLfloat> pt( (normals + 3**ptIds)[0],(normals + 3**ptIds)[1],(normals + 3**ptIds)[2] );
						_point_t<GLfloat> npt = pt.dotproduct( _point_t<GLfloat>(0,0,1) );
						glNormal3f( npt.x, npt.y, npt.z );

						/*
                        _point_t<GLfloat> ppt( (points+ 3**ptIds)[0],(points + 3**ptIds)[1],(points+ 3**ptIds)[2] );
						GLfloat fscale = (134.89-ppt.z)/130.0;
						ppt.update( ppt.x * fscale, ppt.y * fscale, ppt.z);
						glVertex3f( ppt.x, ppt.y, ppt.z );
						*/
                        glVertex3fv(points + 3**ptIds);
                        //glVertex3f( (points + 3**ptIds)[0], (points + 3**ptIds)[1], (points + 3**ptIds)[2] );
                        //cout << (points + 3**ptIds)[0] << "," << (points + 3**ptIds)[1] << "," << (points + 3**ptIds)[2] << "\n";
						,;
						,
                        float *normals = static_cast<float *>(voidNormals););
      break;
...
int vtkTubeHaloMapper::Draw(vtkRenderer *aren, vtkActor *act)
{
...
void vtkTubeHaloMapper::PrintSelf(ostream& os, vtkIndent indent)
{
