...
CTgdataLoader::CTgdataLoader(bool bLoadColor, bool bClassId) :
	CGLoader < vector< GLfloat > > ("tgdata"),
	m_bWithColor(bLoadColor),
	m_bWithClassId( bClassId ),
	m_pClassIds(NULL)
{
}
...
int CTgdataLoader::load(const string& fn)
{
	...
	// for untagged model, simply treat all fibers in a single bundle of ID=1
	int nClsId = 1;

	/**** WE PRESUME THERE IS NO EMPTY LINE IN THE SOURCE FILE ********/

	/* recognize type of the input data model structure: tagged or not */
	istringstream inspector(m_pdata, istringstream::in);
	string secLine; // recognize by simply seeing if the second line contains at least two numbers
	std::getline(inspector, secLine);
	std::getline(inspector, secLine);
	std::vector< std::string > tokens;
	splitstring( secLine, tokens );

	m_bWithClassId = tokens.size() >= 2; 
	cout << (m_bWithClassId?"model with fiber bundle tagged.":"untagged model.") << endl;
	...
	while (istr.good() && icntLine < nLine) {
		...
		m_pClassIds[ icntLine ] = nClsId;
		m_allIds.insert( nClsId );
...
int CTgdataLoader::getClassId(int index)
{
	/*
	if ( ! m_bWithClassId ) { 
		return -1;
	...
	*/
	...
	if ( index >= 0 && index < (int)getSize() ) {
		return m_pClassIds[ index ];
	}

	return -1;
}

int CTgdataLoader::getNumberOfClasses() const
{
	return (int)m_allIds.size();
}
