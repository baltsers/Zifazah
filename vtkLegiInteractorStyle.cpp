		...
		*/
		...
		if ( pipelines ) {
			for (size_t  i = 0; i < pipelines->size(); i++) {
				...
				if ( pipelines->at(i).m_linesort != NULL && 
						pipelines->at(i).m_linesort->GetInput() != NULL && 
						vtkTubeFilter::SafeDownCast(pipelines->at(i).m_streamtube)->GetVaryRadius() > 0) {
					pipelines->at(i).m_linesort->Update();
					pipelines->at(i).m_streamtube->GetInput()->Modified();
				}

				if ( pipelines->at(i).m_dsort != NULL && pipelines->at(i).m_dsort->GetTotalNumberOfInputConnections() >= 1) {
					pipelines->at(i).m_dsort->Update();
					updateActor = true;
				}

				if ( updateActor && pipelines->at(i).m_actor ) {
					pipelines->at(i).m_actor->GetMapper()->GetInput()->Modified();
					//cout << "mapper updated.\n";
				}
			}
		}
