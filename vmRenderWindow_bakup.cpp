...
int CLegiMainWindow::__scenario_explore(const QString&  curline)
{
	QStringList allterms = curline.split("\"");
	...
	if ( "SELECT" == allterms[0].toUpper() ) {
		...
		int inpos = curline.indexOf( "IN", 0, Qt::CaseInsensitive );
		if ( -1 == inpos ) {
			__debugOutput(curline + ": keyword IN missed", DL_FATAL);
			return -1;
		}

		/* 1: deal with conditional expression */
		int midpos = curline.indexOf( allterms[pos-1] ) + allterms[pos-1].count() + 1;
		QString condexp = curline.mid(midpos, inpos - midpos);

		if ( !condexp.startsWith("\"") || !allterms[pos].endsWith("\"") ) {
			//cerr << "here 7 : " << allterms[pos].toStdString() << "\n";
			__debugOutput(condexp + ": invalid conditional expresion", DL_WARNING );
			// syntax error
			return -1;
		}
		condexp.remove('"');
		...
		if ( "ALL" == allterms[pos].toUpper() ) {
			...
			if ( !allterms[pos].startsWith("\"") || !allterms[pos].endsWith("\"") ) {
				// syntax error
				cerr << "here 1!\n";
				__debugOutput(allterms[pos] + ": invalid target expression", DL_WARNING );
				return -1;
			}
