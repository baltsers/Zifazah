...
vtkStandardNewMacro(vtkCanvasWidget);

vtkCxxSetObjectMacro(vtkCanvasWidget,PlaneProperty,vtkProperty);

vtkCanvasWidget::vtkCanvasWidget() : vtkPolyDataSourceWidget()
{
  this->State = vtkCanvasWidget::Start;
  this->EventCallbackCommand->SetCallback(vtkCanvasWidget::ProcessEvents);
...
vtkCanvasWidget::~vtkCanvasWidget()
{
...
void vtkCanvasWidget::SetEnabled(int enabling)
{
  if ( ! this->Interactor )
    {
...
void vtkCanvasWidget::ProcessEvents(vtkObject* vtkNotUsed(object), 
                                   unsigned long event,
                                   void* clientdata, 
                                   void* vtkNotUsed(calldata))
{
	return;
  vtkCanvasWidget* self = reinterpret_cast<vtkCanvasWidget *>( clientdata );
...
void vtkCanvasWidget::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkCanvasWidget::PositionHandles()
{
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
  double *pt2 = this->PlaneSource->GetPoint2();
...
int vtkCanvasWidget::HighlightHandle(vtkProp *prop)
{
...
void vtkCanvasWidget::HighlightNormal(int highlight)
{
  if ( highlight )
    {
    this->ValidPick = 1;
...
void vtkCanvasWidget::HighlightPlane(int highlight)
{
  if ( highlight )
    {
    this->ValidPick = 1;
...
void vtkCanvasWidget::OnLeftButtonDown()
{
  ...
  if (!this->CurrentRenderer || !this->CurrentRenderer->IsInViewport(X, Y))
    {
    this->State = vtkCanvasWidget::Outside;
    return;
    }
  ...
  if ( path != NULL )
    {
    this->State = vtkCanvasWidget::Moving;
    ...
    if ( path != NULL )
      ...
      if ( prop == this->ConeActor || prop == this->LineActor ||
        ...
        {
        this->State = vtkCanvasWidget::Rotating;
        this->HighlightNormal(1);
        }
      else if (this->Interactor->GetControlKey())
        {
        this->State = vtkCanvasWidget::Spinning;
        this->HighlightNormal(1);
        }
      else
        {
...
void vtkCanvasWidget::OnLeftButtonUp()
{
  if ( this->State == vtkCanvasWidget::Outside ||
       this->State == vtkCanvasWidget::Start )
    {
    return;
    }
...
void vtkCanvasWidget::OnMiddleButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if ( path != NULL )
    {
    this->State = vtkCanvasWidget::Pushing;
    this->HighlightPlane(1);
    this->HighlightNormal(1);
    this->HighlightHandle(path->GetFirstNode()->GetViewProp());
    }
...
void vtkCanvasWidget::OnMiddleButtonUp()
{
  if ( this->State == vtkCanvasWidget::Outside ||
       this->State == vtkCanvasWidget::Start )
    {
...
void vtkCanvasWidget::OnRightButtonDown()
{
  int X = this->Interactor->GetEventPosition()[0];
  int Y = this->Interactor->GetEventPosition()[1];
  ...
  if ( path != NULL )
    {
    this->State = vtkCanvasWidget::Scaling;
    this->HighlightPlane(1);
    this->HighlightHandle(path->GetFirstNode()->GetViewProp());
    }
...
void vtkCanvasWidget::OnRightButtonUp()
{
  if ( this->State == vtkCanvasWidget::Outside ||
       this->State == vtkCanvasWidget::Start )
    {
...
void vtkCanvasWidget::OnMouseMove()
{
	cout << "want to move.\n";
	return;
  ...
  // Process the motion
  if ( this->State == vtkCanvasWidget::Moving )
    {
    // Okay to process
    if ( this->CurrentHandle )
      {
  ...
  else if ( this->State == vtkCanvasWidget::Scaling )
    {
    this->Scale(prevPickPoint, pickPoint, X, Y);
    }
  else if ( this->State == vtkCanvasWidget::Pushing )
    {
    this->Push(prevPickPoint, pickPoint);
    }
  else if ( this->State == vtkCanvasWidget::Rotating )
    {
    camera->GetViewPlaneNormal(vpn);
    this->Rotate(X, Y, prevPickPoint, pickPoint, vpn);
    }
  else if ( this->State == vtkCanvasWidget::Spinning )
    {
...
void vtkCanvasWidget::MoveOrigin(double *p1, double *p2)
{
...
void vtkCanvasWidget::MovePoint1(double *p1, double *p2)
{
  //Get the plane definition
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
...
void vtkCanvasWidget::MovePoint2(double *p1, double *p2)
{
  //Get the plane definition
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
...
void vtkCanvasWidget::MovePoint3(double *p1, double *p2)
{
  //Get the plane definition
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
...
void vtkCanvasWidget::Rotate(int X, int Y, double *p1, double *p2, double *vpn)
{
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
  double *pt2 = this->PlaneSource->GetPoint2();
...
void vtkCanvasWidget::Spin(double *p1, double *p2)
{
  // Mouse motion vector in world space
  double v[3];
  v[0] = p2[0] - p1[0];
...
void vtkCanvasWidget::Translate(double *p1, double *p2)
{
...
void vtkCanvasWidget::Scale(double *p1, double *p2, int vtkNotUsed(X), int Y)
{
  //Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
void vtkCanvasWidget::Push(double *p1, double *p2)
{
  //Get the motion vector
  double v[3];
  v[0] = p2[0] - p1[0];
...
void vtkCanvasWidget::CreateDefaultProperties()
{
  // Handle properties
...
void vtkCanvasWidget::PlaceWidget(double bds[6])
{
  int i;
...
void vtkCanvasWidget::SizeHandles()
{
...
void vtkCanvasWidget::SelectRepresentation()
{
  if ( ! this->CurrentRenderer )
    {
    return;
...
void vtkCanvasWidget::SetResolution(int r)
{
  this->PlaneSource->SetXResolution(r); 
  this->PlaneSource->SetYResolution(r); 
}

int vtkCanvasWidget::GetResolution()
{ 
...
void vtkCanvasWidget::SetOrigin(double x, double y, double z) 
{
  this->PlaneSource->SetOrigin(x,y,z);
  this->PositionHandles();
}

void vtkCanvasWidget::SetOrigin(double x[3]) 
{
  this->SetOrigin(x[0], x[1], x[2]);
}

double* vtkCanvasWidget::GetOrigin() 
{
  return this->PlaneSource->GetOrigin();
}

void vtkCanvasWidget::GetOrigin(double xyz[3]) 
{
...
void vtkCanvasWidget::SetPoint1(double x, double y, double z) 
{
  this->PlaneSource->SetPoint1(x,y,z);
  this->PositionHandles();
}

void vtkCanvasWidget::SetPoint1(double x[3]) 
{
  this->SetPoint1(x[0], x[1], x[2]);
}

double* vtkCanvasWidget::GetPoint1() 
{
  return this->PlaneSource->GetPoint1();
}

void vtkCanvasWidget::GetPoint1(double xyz[3]) 
{
...
void vtkCanvasWidget::SetPoint2(double x, double y, double z) 
{
  this->PlaneSource->SetPoint2(x,y,z);
  this->PositionHandles();
}

void vtkCanvasWidget::SetPoint2(double x[3]) 
{
  this->SetPoint2(x[0], x[1], x[2]);
}

double* vtkCanvasWidget::GetPoint2() 
{
  return this->PlaneSource->GetPoint2();
}

void vtkCanvasWidget::GetPoint2(double xyz[3]) 
{
...
void vtkCanvasWidget::SetCenter(double x, double y, double z) 
{
...
// Description:
// Set the center of the plane.
void vtkCanvasWidget::SetCenter(double c[3]) 
{
...
double* vtkCanvasWidget::GetCenter() 
{
  return this->PlaneSource->GetCenter();
}

void vtkCanvasWidget::GetCenter(double xyz[3]) 
{
...
void vtkCanvasWidget::SetNormal(double x, double y, double z) 
{
...
// Description:
// Set the normal to the plane.
void vtkCanvasWidget::SetNormal(double n[3]) 
{
...
double* vtkCanvasWidget::GetNormal() 
{
  return this->PlaneSource->GetNormal();
}

void vtkCanvasWidget::GetNormal(double xyz[3]) 
{
  this->PlaneSource->GetNormal(xyz);
}

void vtkCanvasWidget::GetPolyData(vtkPolyData *pd)
{ 
  pd->ShallowCopy(this->PlaneSource->GetOutput()); 
}

vtkPolyDataAlgorithm *vtkCanvasWidget::GetPolyDataAlgorithm()
{
  return this->PlaneSource;
}

void vtkCanvasWidget::GetPlane(vtkPlane *plane)
{
  if ( plane == NULL )
    {
    return;
...
void vtkCanvasWidget::UpdatePlacement(void)
{
  this->PlaneSource->Update();
  this->PositionHandles();
}
